...
--	WINDOWS -------------------------------------------------------------------
-------------------------------------------------------------------------------
keymap( "n", "HH", "<C-w>h", { desc = "Move to window left" })
keymap( "n", "Hh", '<Cmd>vertical resize -2" <CR>"', { desc = "Shrink window (left)" })
keymap( "n", "Hn", "<C-w>v< C-w>H", { desc = "New window left" })
keymap( "n", "LL", "<C-w>l", { desc = "Move to window right" })
keymap( "n", "Ll", "<Cmd>vertical resize +2 <CR>", { desc = "Grow window (right)" })
keymap( "n", "Ln", "<C-w>v", { desc = "New window right" })
keymap( "n", "QQ", "<Cmd>close<CR>", { desc = "Close window" })
keymap( "n", "Qq", "<Cmd>bdelete<CR>< Cmd>close<CR>", { desc = "Close window and buffer" })
keymap( "n", "<Tab>", "<C-w>w", { desc = "Next window" })
keymap( "n", "<S- Tab>", "<C-w> W", { desc = "Previous window" })
...
vim.keymap.set("n", "Kd", function()
  ...
  -- Temporarily override `d` in normal mode
  vim.keymap.set("n", "d", [["_d]], opts)
  vim.keymap.set("n", "dd", [["_dd]], opts)

  vim.defer_fn(function()
    -- Restore default `d` after 1 keypress (~200ms safe fallback)
    vim.keymap.del("n", "d")
    vim.keymap.del("n", "dd")
  end, 500)
...
keymap( 'n', 'KX', [[:s/\s\+$//<CR>]], { desc = "Trim trailing whitespace" })
keymap( 'n', 'KY', 'ggVGy`"', { desc = "Yank entire buffer without moving" })
keymap( "n", "K;", "mzA;<Esc>`z", { desc = "Add ; to end of line" })
keymap( "n", "K,", "mzA,<Esc>`z", { desc = "Add , to end of line" })
...
keymap("n", ":", ";", { noremap= true, desc = "swap colon types based on use" })
keymap("n", ";", ":", { noremap= true, desc = "swap colon types based on use" })
